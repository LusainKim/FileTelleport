...
#define WM_UPDATEMESSAGE 0x402
#define WM_CREATEWINDOW 0x403

#define AlreadyExistFile	(WPARAM)0x0001
#define NoSaveFile			(WPARAM)0x0002


void SetServerIP(std::string str);
...
typedef struct percentStruct {
	int		nowper;
	double	per_1secDownData;
	double	losstime;
	explicit percentStruct(int iper, double dsec, double dloss) :
		nowper(iper), per_1secDownData(dsec), losstime(dloss)
	{
